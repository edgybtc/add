...
<link href="https://cdn.datatables.net/1.10.19/css/jquery.dataTables.min.css" rel="stylesheet">
  ...
  <li class="nav-item">
                <a class="nav-link active" href="wsetting.php">
                  <span data-feather="file"></span>
...
$(document).ready(function() {
    $('#example').DataTable();
          ...
          <h2>Change Withdrawal settings</h2>
        <?php
		 if (isset($_POST["ch"]) && !empty($_POST["ch"])){
			
$ch=intval($_POST["ch"]);
if($ch==2)$ch=0;
$conn->query("UPDATE setting SET value=$ch WHERE type=\"withdrawal\"");
			

//echo "Withdrawals are set to:<b>$ch</b>";
...
$result = $conn->query("SELECT value FROM setting WHERE type=\"withdrawal\"");
$row = $result->fetch_assoc();
if($row['value']=='1')$set="AUTOMATIC";
if($row['value']=='0')$set="EMAIL CONFIRMATION NEEDED FOR WITHDRAWALS";

		echo '<br>';
		echo '<hr>';
				echo "Current Status:<b>$set $uid</b>";
echo '<br>';
		echo '<hr>';
		?>
<h4>Enter <b>1</b> For Automatic.</h4>
<h4>Enter  <b>2</b> For Email</h4>
<form action="wsetting.php" method="post">
  <div class="form-group">
    <label for="text">Enter here:</label>
