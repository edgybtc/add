...
$conn->query("UPDATE plan_detail SET rate=$rate WHERE plan=\"default\"");
		 ...
		 if (isset($_POST["days"]) && !empty($_POST["days"])) 
		 {
			 ...
			 $conn->query("UPDATE plan_detail SET days=$days WHERE plan=\"default\"");
			 
		 }
		$sql="SELECT rate,days from plan_detail WHERE plan=\"default\"";
		$result=$conn->query($sql);
		$row = $result->fetch_assoc();
		$nrate=$row['rate']; 
		$nday=$row['days'];
		echo "Current Days $nday";
		echo '<br>';
		echo '<hr>';
				echo "Current Rate $nrate";
echo '<br>';
		echo '<hr>';
		?>
